...
class MujocoPointcloudWrapperAdroit(gym.Wrapper):
    ...
    """
    def __init__(self, env, env_name:str, use_point_crop=True, wrist_rgb_size=84):
        super().__init__(env)
        self.env_name = env_name
        self._sim = env.get_mujoco_sim()
        ...
        self.pc_generator = PointCloudGenerator(sim=self._sim,
        ...
        # wrist camera
        cam_ids = [self._sim.model.camera_name2id(c) for c in self._sim.model.camera_names]
        self._wrist_cam_id = None
        for cid in cam_ids:
            if self._sim.model.camera_id2name(cid) == 'wrist':
                self._wrist_cam_id = cid
                break
        self.wrist_rgb_size = wrist_rgb_size
        if self._wrist_cam_id is not None:
            cprint(f"[MujocoPointcloudWrapper] wrist camera found (id={self._wrist_cam_id}), size={wrist_rgb_size}", 'green')
        else:
            cprint(f"[MujocoPointcloudWrapper] no wrist camera in this env", 'yellow')
    ...
    def get_wrist_rgb(self):
        """渲染腕部相机的RGB图像。返回 (H, W, 3) uint8 或 None"""
        if self._wrist_cam_id is None:
            return None
        img = self._sim.render(self.wrist_rgb_size, self.wrist_rgb_size,
                                camera_name='wrist')
    ...
    def step(self, action):
        ...
        if 'adroit' in self.env_name: # adroit uses a namedtuple for obs
            ...
            timestep = ExtendedTimeStepAdroit(step_type=timestep.step_type,
                                         ...
                                         observation_wrist_rgb=wrist_rgb,
